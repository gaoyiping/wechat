...
class ListAction extends Action {
	public function getDefaultView() {
		...
		$sqlcmd = "SELECT `record`.`amount`, `record`.`add_date`, `record`.`accepter`, `user`.`wxname` FROM `ntb_record` AS `record`, `ntb_user` AS `user` WHERE `record`.`type`=11 AND `operation`='{$userid}' AND `user`.`user_id`=`record`.`accepter`";
