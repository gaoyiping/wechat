...
class IndexAction extends Action {
	...
	public function getDefaultView() {
		...
		//总计
		$sql = "select count(id) c, sum(emoneys) m from ntc_rorder t1 where user_id='$userid' $condition order by add_date desc";
		$r = $db->select($sql);
		$total = $r[0]->c;
		//$totalemoneys = $r[0]->m;
		//分页
		...
		$offset = $pager->offset;
		$url = "module=HandleOrder&startdate=$startdate&enddate=$enddate";
		$pagehtml = $pager->num_link($url);
		$_SESSION['handleretail_url'] = "index.php?$url&page=".$pager->cur_page;
		//分页详情
		if($pageto == 'all'){
			$sql = "select * from ntc_rorder where user_id='$userid' $condition ";
		} else {
			$sql = "select * from ntc_rorder where user_id='$userid' $condition order by add_date desc limit $offset,$pagesize";
		}
