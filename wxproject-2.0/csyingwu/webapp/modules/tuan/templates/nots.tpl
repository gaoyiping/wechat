...
{literal}
    function AlertMessageBox()
    {
	        ...
	        location.href='index.php?module=tuan';
	         
    }
        function Showopen(id)
        {
	
            showPopWin("不同意原因","index.php?module=tuan&action=del&id="+id, 460, 280, AlertMessageBox,true,true)
        }
</script>
{/literal}
</head>
...
<table width=95% align="center" border=0>
  <tr><td>
      <a href="index.php?module=tuan"><b>未处理申请</b></a> | <a href="index.php?module=tuan&ok=1" >已处理申请</a> &nbsp;&nbsp;
    申请日期
...
<table align="center" bgcolor="#dedede" border="0" cellpadding="0" cellspacing="0" width="95%">
	<tr><td>
		...
		<td>{$item->add_date}</font></td>
        		...
        		<td>{$item->mobile}</td>
				<td><a href="index.php?module=tuan&action=ok&id={$item->id}" 
          onclick="return confirm('确定要同意吗？');">
          <font color="red">同意</font></a> | 
            <a href="index.php?module=tuan&action=del&id={$item->id}" 
          onclick="return confirm('确定不同意吗？');">
