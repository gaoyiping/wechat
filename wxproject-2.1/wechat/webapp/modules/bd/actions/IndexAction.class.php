	...
	public function execute(){
		...
		if(!$r){
			...
			echo "<script type='text/javascript'>" .
						"alert('用户不存在！');" .
						"location.href='index.php?module=bd'; </script>";
			return ;
		}
		...
		if($type=='user'){
			...
			do{
				...
				$sql="insert into ntb_log (userid,event,utype) values ('$userid','会员 $adminid 为 $userid 充值购物币:$nummoney',1) ";
				$r= $db -> update($sql);
			...
			if($rollback){
				...
				echo "<script type='text/javascript'>" .
						"alert('转币成功！');" .
						"location.href='index.php?module=bd&action=list'; </script>";
			}
			//over	
		}
		return;
