...
class groupAction extends Action {
	...
	public function execute(){
		...
		$percent = array(
			0 => array(null),
			1 => array(null, 0.3, 0.2),
			6 => array(null, 0.3, 0.2, 0.1, 0.03),
			2 => array(null, 0.3, 0.2, 0.1, 0.03, 0.03, 0.03, 0.03),
			3 => array(null, 0.3, 0.2, 0.1, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03),
			4 => array(null, 0.3, 0.2, 0.1, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03),
			5 => array(null, 0.3, 0.2, 0.1, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03),
			7 => array(null, 0.2, 0.2, 0.1, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03, 0.03),
		);
		$outlvel = array(
			0 => 0,	// 见习店小二
			1 => 2,	// 店小二
		...
		while ($relation_level <= 11) {
			...
			if ($relation) {
				...
				if ($relation_level <= $outlvel[$uplevel]) {
					$money = round($pv * $percent[$uplevel][$relation_level], 2);
				}
				...
				$sqlcmd = "SELECT * FROM `ntb_user` WHERE `user_id`='{$pid}'";
				$relation = $db->getone($sqlcmd);
				$score_array = array(2, 3, 4, 5);
				if (in_array($uplevel, $score_array)) {
					$sqlcmd = "SELECT SUM(`money`) AS `score` FROm `ntb_money_point` WHERE (`type`=0 OR `type`=1) AND `userid`='{$pid}'";
					$result = $db->select($sqlcmd);
					$score = $result[0]->score;

					// 掌柜 -> 东家
					if ($uplevel == 2 && $relation['t1'] + $relation['t2'] + $relation['t3'] >= 600 && $score >= 30000) {
					...
					// 东家 -> 富豪
					if ($uplevel == 3 && $relation['t1'] + $relation['t2'] + $relation['t3'] >= 1000 && $score >= 50000) {
					...
					// 富豪 -> 大富豪
					if ($uplevel == 4 && $relation['t1'] + $relation['t2'] + $relation['t3'] >= 3000 && $score >= 200000) {
					...
					// 大富豪 -> 董事
					if ($uplevel == 5 && $relation['t1'] + $relation['t2'] + $relation['t3'] >= 5000 && $score >= 200000) {
						$sqlcmd = "UPDATE `ntb_user` SET `uplevel`=7 WHERE `user_id`='{$pid}'";
						$result = $db->update($sqlcmd);
						if ($result == -1) {
							$db->rollback();
							echo '<script type="text/javascript">alert("{$pid}升级董事失败！请重试！");location.href="index.php?module=cart";</script>';
							return;
						}
					}
				}
				// 伙计 -> 掌柜
				if (($uplevel == 6 || $uplevel == 1) && $relation['t1'] >= 30) {
					...
					// 店小二 -> 伙计
				} else if ($uplevel == 1 && $relation['t1'] >= 15) {
					$sqlcmd = "UPDATE `ntb_user` SET `uplevel`=6 WHERE `user_id`='{$pid}'";
					$result = $db->update($sqlcmd);
					if ($result == -1) {
						$db->rollback();
						echo '<script type="text/javascript">alert("{$pid}升级伙计失败！请重试！");location.href="index.php?module=cart";</script>';
						return;
					}
				}
				$pid = $relation['pid'];
