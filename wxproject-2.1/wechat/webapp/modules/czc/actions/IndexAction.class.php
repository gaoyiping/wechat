	...
	public function execute(){
		...
		if(!$r){
			...
			echo "<script type='text/javascript'>" .
						"alert('用户不存在！');" .
						"location.href='index.php?module=czc'; </script>";
			return ;
		}
		
		
		//用户信息
		$sql = "select user_name, c_money  from ntb_user where user_id = '$adminid'";
		$r = $db->select($sql);
		$z_money = $r[0]->c_money;
		...
		if($type=='user'){
			...
			do{
				...
				$sql="insert into ntb_log (userid,money,event,utype) values ('$userid',$nummoney,'会员 $adminid 为 $userid 充值充值币:$nummoney',1) ";
				$r= $db -> update($sql);
			...
			if($rollback){
				...
				echo "<script type='text/javascript'>" .
						"alert('转币成功！');" .
						"location.href='index.php?module=czc&action=list'; </script>";
			}
			//over	
		}
		return;
