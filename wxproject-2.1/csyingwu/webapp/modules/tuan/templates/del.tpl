...
<body scroll="yes">
<form name="form1" action="index.php?module=TiXiandanbao&action=del" method="post" onsubmit="return check(this)">
<input type="hidden" name="id" value="{$info->id}" />
<input type="hidden" name="userid" value="{$info->operation}" />
<input type="hidden" name="money" value="{$info->amount}" />
<input type="hidden" name="shouxufei" value="{$info->amount*0.05}" />
<div style='font-size:14px; padding:10px;'>
提现账号：{$info->operation} 提现金额：￥{$info->amount} 
</div>
<div style='font-size:14px; padding:10px;'>
