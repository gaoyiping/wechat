...
class ListAction extends Action {
	public function getDefaultView() {
		...
		$sql = "select sNo  from  ntb_fuxiaojiesuan  group by sNo order by  sNo desc";
		...
		$sql = "select count(a.id) c,sum(f_money) as zongjin  from ntb_fuxiaojiesuan a left join ntb_user b on a.userid=b.user_id where 1=1  $condition order by  a.id ";
		...
		if ($r1) {
			...
			$offset = $pager->offset;
			$url = "module=fuxiaojiang&action=list&sqishu=$sqishu&user_id=$user_id";
			$pagehtml = $pager->num_link ( $url );
			$_SESSION ['daili_url'] = "index.php?$url&page=" . $pager->cur_page;
			// 详情
			$sql = "select a.*,b.user_name,b.usertype,b.usertype,b.card_name,b.provcity,b.card_type,b.card_number,b.mobile from ntb_fuxiaojiesuan a left join ntb_user b on a.userid=b.user_id" . " where 1=1 $condition  order by  a.id  ";
